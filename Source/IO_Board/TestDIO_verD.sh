...
DO_MON_TEST() 
{
	...
	do
		...
		if [ !  "${DO_MON_VAL[$cnt]}" = "0x0" ];					#Check DO1 Mon change to 1
		then
...
DI_TEST_PIN_0()
{
	...
	#---------------------------------------------#
	for cnt in $(seq 0 3)		#Turn off DO1~4
	do
		$DevMem ${DO_A_Array[$cnt]} b 0 |grep readback |grep null;$DevMem ${DO_B_Array[$cnt]} b 0 |grep readback |grep null	#Set DO_A and DO_B to 0 and 0
		DO_MON_VAL[$cnt]=$($DevMem ${DO_MON_Array[$cnt]} b| grep Value | cut -d ' ' -f6)
		if [ !  "${DO_MON_VAL[$cnt]}" = "0x0" ];					#Check DO1 Mon change to 1
	...
	do
		...
		then
			#echo "Check test pin is 0 then DIA adn DIB is 0 and 1."
			dio_err=$((dio_err+1))
		fi 
	...
	#---------------------------------------------#	
	for cnt in $(seq 0 3)		#Turn on DO1~4
	do
		$DevMem ${DO_A_Array[$cnt]} b 1 |grep readback |grep null;$DevMem ${DO_B_Array[$cnt]} b 0 |grep readback |grep null	#Set DO_A and DO_B to 0 and 1
		DO_MON_VAL[$cnt]=$($DevMem ${DO_MON_Array[$cnt]} b| grep Value | cut -d ' ' -f6)
		if [ !  "${DO_MON_VAL[$cnt]}" = "0x2" ];					#Check DO1 Mon change to 1
		then
...
DI_TEST_PIN_1()
{
	...
	do
		DI_A_Val[$cnt]=$($DevMem ${DI_A_Array[$cnt]} b| grep Value | cut -d ' ' -f6);DI_B_Val[$cnt]=$($DevMem ${DI_B_Array[$cnt]} b| grep Value | cut -d ' ' -f6)
		if [ !  "${DI_A_Val[$cnt]}" = "0x0" ] & [ !  "${DI_B_Val[$cnt]}" = "0x1" ];
		then
			#echo "Check test pin is 0 then DIA and DIB is 0 and 1."
			dio_err=$((dio_err+1))
		fi 
